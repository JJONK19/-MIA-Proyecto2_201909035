...
func Mkfs(parametros *[]string, discos *[]Disco, salidas *[6]string){
	...
	intTemp = posInicio + int(binary.Size(Sbloque{})) + (n)
	copy(nuevo.S_bm_block_start[:], strconv.Itoa(intTemp))
	intTemp = posInicio + int(binary.Size(Sbloque{})) + (n) + (n * 3)
	copy(nuevo.S_inode_start[:], strconv.Itoa(intTemp))
	intTemp = posInicio + int(binary.Size(Sbloque{})) + (n) + (n * 3) + (int(binary.Size(Inodo{})) * n)
