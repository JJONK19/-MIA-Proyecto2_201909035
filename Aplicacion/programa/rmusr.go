...
func Rmusr(parametros *[]string, discos *[]Disco, sesion *Usuario, salidas *[6]string) {
	...
	var paramFlag     = true          //Indica si se cumplen con los parametros del comando
	var required      = true          //Indica si vienen los parametros obligatorios
	var nombre        = ""           //Atributo name
	var	diskName      = ""           //Nombre del disco
	var	posDisco      = -1           //Posicion del disco dentro del vector
	var	posParticion  = -1           //Posicion de la particion dentro del vector del disco
	var	posInicio     int            //Posicion donde inicia la particion
	var	posLectura    int            //Para determinar la posicion de lectura en disco
	var	inodo_buscado = -1           //Numero de Inodo del archivo users.txt
	...
	var	larchivo      Barchivos      //Para el manejo de bloques de archivos
	var	texto         = ""           //Para almacenar el contenido del archivo de usuarios
	var	existe_usuario = false        //Indica si se encontró el usuario
	...
	if !required {
		(*salidas)[0] += "ERROR: La instrucción rmusr carece de todos los parametros obligatorios.\n"
		...
		if len(atributos) == 5 { //Los usuarios tienen cinco parametros
			if atributos[0] != "0" {
				if atributos[3] == nombre {
					editar := "0," + atributos[1] + "," + atributos[2] + "," + atributos[3] + "," + atributos[4]
					existe_usuario = true
					lineas[i] = editar
				}
			}
	...
	if !existe_usuario {
		(*salidas)[0] +=  "ERROR: El usuario que desea eliminar no existe.\n"
		return
	}
	...
	(*salidas)[0] += "MENSAJE: Usuario eliminado correctamente.\n"
}